...
export const RegistrationPasswordCriteria = ({ password }: PasswordCriteriaProps) => {
      ...
      {criteria.map(({ key, label }) => {
        ...
        return (
          <View key={key} style={styles.criteriaRow}>
            <FontAwesome color={isMet ? '#2E9B45' : '#979797'} name={isMet ? 'check-circle' : 'circle-o'} size={13} />
            <ThemedText style={[styles.criteriaText, isMet && styles.criteriaMet]}>{label}</ThemedText>
          </View>
        );
      })}
    </View>
