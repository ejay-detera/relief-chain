// prepare-wallet-provision Edge Function.
//
// Builds the exact sponsored RCPHP trustline transaction a beneficiary's phone
// wallet must sign to become able to hold RCPHP under AUTH_REQUIRED. Ensures the
// wallet account exists on testnet (Friendbot) first, and short-circuits if the
// wallet already has an authorized trustline. Returns ONLY the unsigned
// transaction + the expected signer; the phone signs its own changeTrust.
//
...
const prepareWalletProvision = async (scope: EdgeRequestScope): Promise<Response> => {
  const { request, context, correlationId } = scope;
  ...
  // Ensure the wallet account exists (testnet Friendbot); a changeTrust needs it.
  let account;
  try {
...
serveEdge((request) => handleEdgeRequest(request, prepareWalletProvision));
